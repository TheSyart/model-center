...
Gemini 项目识别与官方 Gemini CLI `9c1b0a6` 保持一致：缺少当前项目时保留 `ineligibleTiers` 的具体原因（例如地区、网络、年龄、账号验证），不能一律提示手工填写项目 ID；尚未初始化的账号先执行默认套餐的 `onboardUser`，由 Google 决定是否返回托管项目。个人 Gmail 通常不需要手动项目，但账号资格与许可证仍由 Google 判断。遇到失败后需重新生成授权链接，旧授权会话不可重复提交。当前修复的回归测试使用模拟上游响应，实际账号的限制须以再次授权返回的错误为准。
