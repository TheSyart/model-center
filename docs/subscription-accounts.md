...
新增管理入口 `/subscriptions`，原生集成到 Model Center 的 Next.js 服务中，无需部署 CLIProxyAPI sidecar。当前登录入口为 Claude Code、Codex、Antigravity CLI（反重力），参考固定版本 CLIProxyAPI 的独立授权和请求执行协议；**没有执行真实账号的登录、额度查询或推理验收**。
...
2. 授权登录厂商集中在订阅页，Claude/Codex 复用服务商目录图标，Antigravity 使用独立官方图标。**GitHub Copilot、xAI (Grok)** 保留“尚未开放”占位，不发起未实现的登录；API Key 服务商选择器不再展示 OAuth-only 预设。选择 **登录 Claude Code / Codex / Antigravity CLI**，生成链接后打开官方授权页。
3. 三家授权后，复制浏览器地址栏的完整本地回调 URL。页面显示 localhost 无法访问不影响复制；本应用使用手动粘贴回调，无需开启回调监听端口。Antigravity 的回调为 `http://localhost:51121/oauth-callback`，必须粘贴含 code 和 state 的完整地址，不接受裸授权码。返回 Model Center，粘贴并完成登录。
...
- `lib/subscriptions/oauth.ts`：Claude/Codex PKCE、所有客户端的 state、代码交换、刷新和 Google 项目初始化。
- `quota.ts`：现有三家及旧 Gemini 额度解析；`store.ts` / `lifecycle.ts`：加密、会话、版本、刷新和快照。
- `gateway.ts`：固定官方 URL、OAuth 请求头、Codex SSE 收集、Antigravity 包装及 Gemini 兼容。
...
数据库使用 `subscription_accounts`、`subscription_oauth_sessions`、`subscription_provider_links` 三张表。旧账号表通过事务重建扩展 vendor 约束，保留所有旧账号和网关关联；旧 `gemini` 不改成 `antigravity`，不混用凭据，旧账号保留查询/刷新/删除能力，新建和重新登录入口停用。保留原 provider schema；OAuth 凭据不存放在 API Key 字段。加密复用 AES-256-GCM；浏览器列表只返回脱敏视图，授权会话由 HttpOnly / SameSite=Strict Cookie 绑定，10 分钟有效且只能使用一次。取消后清除会话和浏览器粘贴值。
...
- CLIProxyAPI **MIT**，SHA [`7fa443dc8bf8ca2f1ffd81c2472deb31b097b697`](https://github.com/router-for-me/CLIProxyAPI/tree/7fa443dc8bf8ca2f1ffd81c2472deb31b097b697)：Claude/Codex/Antigravity OAuth、请求执行及 Antigravity 图标。
