...
- **订阅账号**：Claude Code、Codex、Antigravity CLI OAuth 登录与 GitHub Copilot 设备码登录，登录后自动拉取账号可用模型并接入网关；多账号额度窗口、凭据自动刷新、独立网关服务商与别名回退。入口 `/subscriptions`；使用方法、部署配置和真实账号待验范围见 [订阅账号文档](docs/subscription-accounts.md)。
